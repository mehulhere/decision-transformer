...
def experiment(
    ...
    def get_batch(batch_size=256, max_len=K):
        ...
        s, a, r, d, rtg_list, timesteps_list, mask = [], [], [], [], [], [], []
        ...
        rtg_tensor = torch.from_numpy(np.concatenate(rtg_list, axis=0)).to(dtype=torch.float32, device=device)
        t_tensor = torch.from_numpy(np.concatenate(timesteps_list, axis=0)).to(dtype=torch.long, device=device)
        m_tensor = torch.from_numpy(np.concatenate(mask, axis=0)).to(device=device)

        return s, a, r, d, rtg_tensor, t_tensor, m_tensor
    ...
    def eval_episodes(target_rew):
        def fn(model):
            ...
            for _ in range(num_eval_episodes):
                with torch.no_grad():
                    if model_type == 'dt':
                        # IMPORTANT: slice rtg as in original code
    ...
    if model_type == 'dt':
        # Use rtg[:,:-1] when calling forward in the trainer to match sequence length
...
if __name__ == '__main__':
    ...
    experiment(experiment_prefix, variant=VARIANT)
